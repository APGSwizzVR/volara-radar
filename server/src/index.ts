...
type Aircraft={id:string;callsign:string;cid:number;lat:number;lon:number;altitude:number;groundspeed:number;heading:number;verticalSpeed:number;squawk:string;aircraftType:string;departure:string;destination:string;route:string;source:'VATSIM';updatedAt:number};
...
async function refresh(){
  ...
  state={updatedAt:now,controllers:Array.isArray(data.controllers)?data.controllers.length:0,aircraft:pilots.map((p:any)=>({
    id:`vatsim:${p.cid}`,callsign:text(p.callsign).trim(),cid:num(p.cid),lat:num(p.latitude),lon:num(p.longitude),altitude:num(p.altitude),
    groundspeed:num(p.groundspeed),heading:num(p.heading),verticalSpeed:num(p.vertical_speed),squawk:text(p.transponder),
    aircraftType:text(p.flight_plan?.aircraft_short),departure:text(p.flight_plan?.departure),destination:text(p.flight_plan?.arrival),route:text(p.flight_plan?.route),source:'VATSIM',updatedAt:now
...
app.get('/api/weather',async(req,reply)=>{
  ...
  const lat=Number(q.lat),lon=Number(q.lon),key=process.env.OPENWEATHER_API_KEY;
  ...
  const res=await fetch(url);const data=await res.json();return reply.code(res.ok?200:res.status).send(data);
});
...
app.log.info(`Volara Radar listening on ${PORT}`);

async function loop(){try{await refresh()}catch(e){app.log.error(e)}setTimeout(loop,Math.max(3000,5000-(Date.now()-lastFetch)))}
loop();
